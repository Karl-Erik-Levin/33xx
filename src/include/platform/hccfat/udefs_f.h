...
#define FN_MAXVOLUME	1	/* maximum number of volumes */
#ifndef BOOTLOADER
#define FN_MAXTASK		2	/* maximum number of task for firmware */
#else
#define FN_MAXTASK		1	/* maximum number of task  for bootloader */
#endif

#define FN_MAXPATH		64	/* maximum allowed filename or pathname */

#define FN_CURRDRIVE	0	/* setting the current drive at startup (-1 means no default current drive)*/

// DATATON
#ifndef BOOTLOADER
#define FN_MUTEX_TYPE xSemaphoreHandle
#else
...
#define F_MAXFILES   6    /* maximum number of files */

#define F_MAXSEEKPOS 0	   /* number of division of fast seeking */
...
#define __malloc(x) pvPortMalloc(x)
#define __free(x) vPortFree(x)
#endif
...
#define FATCACHE_READAHEAD 4	/* max. 256 */
#endif

#if F_LONGFILENAME
#ifdef DIRCACHE_ENABLE
#define DIRCACHE_SIZE 8	/* max. 32 (<=max. cluster size) */
#endif
#endif

#define WR_DATACACHE_SIZE 1	/* min. 1 !!!! */
