...
static FN_FILEINT *_f_getintfile(void)
{
	...
	if (f_mutex_get(&fat_gmutex)) return 0;
	
	for (a=0; a<F_MAXFILES; a++)
	{
		if (f_filesystem.files[a].mode==FN_FILE_CLOSE)
		{
			f_filesystem.files[a].file.reference=(void *)((f_filesystem.drvbldnum<<F_MAXFILES_SHIFT)+a);
			
			f_filesystem.files[a].WrDataCache.N=0;
			f_filesystem.files[a].WrDataCache.ptr=NULL;
			
			f_filesystem.files[a].mode=FN_FILE_LOCKED; /* for avoiding reentrancy */
 ...
 *
 ***************************************************************************/
unsigned long fn_getfilesize(F_MULTI *fm, FN_FILE *filehandle)
{
 ...
 *
 ***************************************************************************/
FN_FILE *fn_openopt(F_MULTI *fm,const unsigned long startcluster,const unsigned long filesize)
{
	F_VOLUME *vi;
	char m_mode=FN_FILE_RD;
	FN_FILEINT *f=0;
	int drivenum = 0;	/* One drive only */
	int ret;
	
	if (_f_getvolume(fm,drivenum,&vi)) 
	{
		F_SETLASTERROR_NORET(F_ERR_INVALIDDRIVE);
		return 0; /* cant open any */
	}
	...
	/* Mode is READONLY */
	/* dirpos (f->dirpos) and directory entry (f->de) isn't set */
	/* Therefore we can't check if file is locked for reading */
	/* Set dirpos to 0 */
	f->dirpos.pos = 0;
	f->dirpos.sector = 0;
	
	f->startcluster = startcluster;
