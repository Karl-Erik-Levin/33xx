...
#include "Platform/hccfat/chkdsk.h"
#include "MFLib\Dataton_Types.h"
#include "drv/CpuDrv.h"
...
static int chkdsk_savelostchain (F_MULTI *fm, F_VOLUME *vi, unsigned char *ctbl, unsigned long first_cluster, unsigned long csize, int param)
{
  ...
  if (chkdsk_getlostname(fm,"/",lost_name)==0)
  {
    ...
    if (_f_addentry(vi,&fsname,&dirpos,&direntry)==0)
    {
		...
		_f_set16bitl(direntry->cdate,date);
		_f_set16bitl(direntry->ctime,time);
...
int fn_checkdisk (F_MULTI *fm,int drivenum, int param)
{
  F_VOLUME *vi;
  F_POS first_pos;
  int rc;
  ...
  CPUWatchDogKick();		// Dataton WD kick
