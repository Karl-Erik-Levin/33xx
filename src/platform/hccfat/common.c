...
#include "Platform/hccfat/debug.h"
...
#ifndef HCC_UNICODE
#if (!F_LONGFILENAME)
	return (char*)("HCC_FAT ver:3.50");
#else
	return (char*)("HCC_FAT_LFN ver:3.50");
#endif
#else
	return (char*)("HCC_FAT_LFN_UNI ver:3.50");
#endif
}
...
int _f_checkstatus(const F_VOLUME *vi,long *pstatus)
{
	/* reset status */
...
static int _f_drvwritesector(F_DRIVER *driver, void *data, unsigned long sector)
{
	...
	while (retry--)
	{
		...
		if (!ret) break; /* if success stop retrying */
	...
	if (ret==-1) return F_ERR_CARDREMOVED;
...
int _f_writesector(F_VOLUME *vi, void *data, unsigned long sector, int cnt)
{
	...
	if (status & (F_ST_WRPROTECT)) return F_ERR_WRITEPROTECT;
	...
	if (cnt==1 || (!vi->driver->writemultiplesector))
	{
		...
		{
			/* write data */
			ret=_f_drvwritesector(vi->driver,(char *)data+i*(vi->bootrecord.bytes_per_sector),sector);
...
int _f_readsector(F_VOLUME *vi,void *data, unsigned long sector,int cnt)
{
	...
	if (!vi->driver->readsector) return F_ERR_ACCESSDENIED;
	...
	if (cnt==1 || (!vi->driver->readmultiplesector)) {
		...
		for (i=0;i<cnt;i++,sector++)
		{
			ret=_f_drvreadsector(vi->driver,(char *)data+i*(vi->bootrecord.bytes_per_sector),sector);
			if (ret)
			{
				if (ret==F_ERR_CARDREMOVED)
				{
		...
		do
		{
			...
			if (i<vi->fatcache.N)
			{
				  ...
				  ;
				{
					...
					{
						int ret=_f_writesector(vi,(vi->fatcache.data)+(i*(vi->bootrecord.bytes_per_sector)),fatsector,j-i);
						if (ret) return ret;
...
static int _f_getfatsector(F_VOLUME *vi,unsigned long sector)
{
	...
	if (vi->fatsector!=sector)
	{
		...
		if (pos==vi->fatcache.N)
		{
			...
			for (a=0; a<vi->bootrecord.number_of_FATs; a++)
			{
				if (!_f_readsector(vi,ptr,fatsector,readahead))
				{
					vi->fat=ptr;
					for (i=0;i<readahead;i++) (vi->fatcache.dsc+(pos+i))->sector=fatsector+(unsigned long)i;
...
static void _f_close_drive_files (int drivenum)
{
	...
	for (a=0; a<F_MAXFILES; a++)
	{
	    ...
	    if (f->drivenum==drivenum)
		{
			...
			{
#ifdef USE_MALLOC
				if (f->WrDataCache.pos) __free(f->WrDataCache.pos);
...
int _f_getvolume(F_MULTI *fm,int drivenum,F_VOLUME **pvi)
{
	...
	case F_STATE_WORKING:
		if (!_f_checkstatus(vi,0)) return F_NO_ERROR;
...
{
	if (!vi->sectornum) return F_ERR_INVALIDSECTOR;
...
int _f_getcurrsector(F_VOLUME *vi,FN_FILEINT *f,char *ptr,unsigned int *cnt)
{
	int ret;
	...
	while (f->pos.cluster>=2 && f->pos.cluster<F_CLUSTER_RESERVED)
	{
		if (f->pos.sector<f->pos.sectorend)
		{
			...
			return _f_readsector(vi,ptr,fsector,(int)N);
		}
...
#ifdef FATBITFIELD_ENABLE
						if (cou2+1==(vi->bootrecord.bytes_per_sector)/4)
						{
							cptr=vi->fat; /* check again if its all used */
							for (cou2=0; cou2<(vi->bootrecord.bytes_per_sector)/4; cou2++, cptr+=4)
							{
								if (_f_get32bitl(cptr)==0)
								{
...
int _f_writezeros(F_VOLUME *vi,F_MULTI *fm,FN_FILEINT *f,unsigned long num)
{
	...
	while (num)
	{
		...
		if (size>(vi->bootrecord.bytes_per_sector)) size=(vi->bootrecord.bytes_per_sector);

		if ((long)size!=fn_write(fm,vi->sectorbuffer,1,(long)size,(FN_FILE *)(f->file.reference)))
		{
#ifdef USE_MALLOC
