...
/*---------------------------------------------------------------------------------
 * Implementation of functions defined in portable.h for the Atmel AT91SAM7S port
 *---------------------------------------------------------------------------------*/
...
#define portINITIAL_SPSR				( ( portSTACK_TYPE ) 0x1f ) /* System mode, THUMB mode, interrupts enabled. */
...
static Boolean osIsSleeping = true;
...
#elif (FREERTOS_TIMER == TC1)
	tmrInit();
#else
  #error port.c: Illegal FREERTOS_TIMER value
#endif
}
/*-----------------------------------------------------------*/
...
{
#if (FREERTOS_TIMER == PIT)
	AT91C_BASE_PITC->PITC_PIMR= 0;		// Stop PIT
#else	// FREERTOS_TIMER == TC1
	osIsSleeping = true;
#endif
}
/*-----------------------------------------------------------*/
...
#if (FREERTOS_TIMER == PIT)
	// Start PIT
	AT91C_BASE_PITC->PITC_PIMR = AT91C_PITC_PITEN | AT91C_PITC_PITIEN | portPIT_COUNTER_VALUE;
#else	// FREERTOS_TIMER == TC1
	osIsSleeping = false;
#endif
}
...
/* FreeRTOS timer interrupt, called every ms.
   When we use PIT we are called from vPortPreemptiveTick in portasm.asm
   When we use TC1 we are called from usTimerISR in tmrDrv.c
*/
void vFreeRTOSTimerCall(void)
{
	if (!osIsSleeping) {			// NOT sleeping
		vTaskIncrementTick();		// Increment the tick count - this may wake a task
		vTaskSwitchContext();		// Select the next task to execute
	}
}
